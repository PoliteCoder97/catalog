...
public class PanelPersonListActivity extends AppCompatActivity {
  ...
  public void onPanelPersonListLongEventListener(PanelPersonListLongEventListener event) {
    ...
    builder.setPositiveButton("Yes", (dialogInterface, i) -> {
      ...
      params.put("token", Collections.singletonList(App.preferences.getString(Consts.TOKEN, "")));
      params.put("refresh_token", Collections.singletonList(App.preferences.getString(Consts.REFRESH_TOKEN, "")));
      params.put("personId", Collections.singletonList(String.valueOf(event.getPerson().getId())));

      Ion.with(PanelPersonListActivity.this)
        .load(Utils.checkVersionAndBuildUrl(Consts.DELETE_USER))
        .setBodyParameters(params)
        .asString()
        .setCallback(new FutureCallback<String>() {
          @Override
          public void onCompleted(Exception e, String result) {
            ...
            Log.i("DELETE‌USER", "result: " + result);
            try {
              JSONObject jsonObject = new JSONObject(result);
              if (jsonObject.getBoolean("error")) {
                Toast.makeText(PanelPersonListActivity.this, jsonObject.getString("message"), Toast.LENGTH_SHORT).show();
              ...
              personList.remove(event.getPerson());
              App.database.getPersonDao().delete(event.getPerson());
              panelPersonListAdapter.notifyDataSetChanged();
